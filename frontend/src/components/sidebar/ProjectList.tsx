...
export function ProjectList() {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!confirm('确定删除该项目及所有关联素材？')) return;
    try {
  ...
  if (!projects.length) {
    return <div style={{ padding: 'var(--sp-3) var(--sp-4)', font: '400 11px var(--font)', color: 'var(--text-3)' }}>暂无项目</div>;
  }
    ...
    <div>
      {projects.map((p) => (
        ...
        >
          ...
          <button className="sidebar-project-del" onClick={(e) => handleDelete(p.id, e)} title="删除">
